...
NAN_METHOD(GetEventInfo) {
  ...
  // Arg 1
  cl_event_info param_name = Nan::To<uint32_t>(info[1]).ToChecked();
...
NAN_METHOD(SetUserEventStatus) {
  ...
  cl_int exec_status=Nan::To<uint32_t>(info[1]).ToChecked();
...
NAN_METHOD(GetEventProfilingInfo) {
  ...
  cl_profiling_info param_name = Nan::To<uint32_t>(info[1]).ToChecked();
...
NAN_METHOD(SetEventCallback)
{
  ...
  cl_int callbackStatusType = Nan::To<int32_t>(info[1]).ToChecked();
